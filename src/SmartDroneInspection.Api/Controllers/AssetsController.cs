...
public class AssetsController(IMediator mediator) : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<PagedResult<AssetDto>>> GetAssetsAsync([FromQuery] AssetListQuery filters, CancellationToken ct)
    {
    ...
    [HttpGet("{id:guid}")]
    public async Task<ActionResult<AssetDto>> GetByIdAsync(Guid id, CancellationToken ct)
    {
