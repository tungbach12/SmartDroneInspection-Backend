...
public sealed class DefectConfiguration : IEntityTypeConfiguration<Defect>
{
    public void Configure(EntityTypeBuilder<Defect> builder)
    {
        ...
        builder.Property(x => x.Id).HasVogenConversion(); builder.ConfigureAudit(); builder.Property(x => x.DefectNumber).HasMaxLength(40); builder.Property(x => x.Title).HasMaxLength(250).IsRequired(); builder.Property(x => x.Description).HasColumnType("text").IsRequired(); builder.Property(x => x.Severity).HasConversion(SmartEnumStringConverter.Create<DefectSeverity>()).HasMaxLength(32); builder.Property(x => x.Category).HasConversion(SmartEnumStringConverter.Create<DefectCategory>()).HasMaxLength(40); builder.Property(x => x.RepairRecommendation).HasColumnType("text"); builder.Property(x => x.RepairPriority).HasConversion(SmartEnumStringConverter.Create<RepairPriority>()).HasMaxLength(32); builder.Property(x => x.Status).HasConversion(SmartEnumStringConverter.Create<DefectStatus>()).HasMaxLength(32); builder.Property(x => x.EstimatedRepairCost).HasPrecision(15, 2);
