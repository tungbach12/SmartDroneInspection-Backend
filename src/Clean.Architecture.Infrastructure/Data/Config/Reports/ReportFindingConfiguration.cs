...
public sealed class ReportFindingConfiguration : IEntityTypeConfiguration<ReportFinding>
{
    public void Configure(EntityTypeBuilder<ReportFinding> builder)
    {
        ...
        builder.Property(x => x.Id).HasVogenConversion(); builder.Property(x => x.Description).HasColumnType("text").IsRequired(); builder.Property(x => x.Severity).HasConversion<string>().HasMaxLength(32); builder.Property(x => x.LocationNote).HasMaxLength(500); builder.Property(x => x.BoundingBox).HasColumnType("jsonb"); builder.Property(x => x.ConfidenceScore).HasPrecision(5, 4);
