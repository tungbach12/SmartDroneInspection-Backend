...
public sealed class MissionFlightLogConfiguration : IEntityTypeConfiguration<MissionFlightLog>
{
    ...
    {
        builder.ConfigureBase("mission_flight_logs");
        builder.Property(x => x.Id).HasVogenConversion(); builder.Property(x => x.LogType).HasConversion(SmartEnumStringConverter.Create<FlightLogType>()).HasMaxLength(32); builder.Property(x => x.Content).HasColumnType("jsonb").IsRequired();
