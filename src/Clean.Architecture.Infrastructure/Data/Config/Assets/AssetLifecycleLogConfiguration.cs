...
public sealed class AssetLifecycleLogConfiguration : IEntityTypeConfiguration<AssetLifecycleLog>
{
    public void Configure(EntityTypeBuilder<AssetLifecycleLog> builder)
    {
        ...
        builder.Property(x => x.Id).HasVogenConversion(); builder.Property(x => x.FromStatus).HasConversion(SmartEnumStringConverter.CreateNullable<AssetStatus>()).HasMaxLength(32);
        builder.Property(x => x.ToStatus).HasConversion(SmartEnumStringConverter.Create<AssetStatus>()).HasMaxLength(32).IsRequired(); builder.Property(x => x.ChangedAt).IsRequired();
